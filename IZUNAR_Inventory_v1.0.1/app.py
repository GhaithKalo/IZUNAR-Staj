...
def borrow(comp_id):
    ...
    component.quantity -= amount
    log = BorrowLog(user_id=current_user.id, comp_id=comp_id, action='borrow', amount=amount)
    db.session.add(log)
    db.session.commit()
...
@app.route('/projelerim/yeni', methods=['GET', 'POST'])
@login_required
def create_project():
    # İstersen kategori bazlı grupla
    all_components = Component.query.order_by(Component.type, Component.name).all()
    components_by_type = {}
    for c in all_components:
        components_by_type.setdefault(c.type or "Diğer", []).append(c)
    ...
    if request.method == 'POST':
        ...
        # component checkbox isimleri 'component_{{id}}', adet input isimleri 'amount_{{id}}' olacak
        selected_components = []
        for c in all_components:
            if request.form.get(f'component_{c.id}') == 'on':  # checkbox işaretli mi
                try:
                    amount = int(request.form.get(f'amount_{c.id}', '0'))
                except ValueError:
                    amount = 0
                if amount > 0:
                    selected_components.append( (c.id, amount) )

        # Yeni proje oluştur
        project = Project(name=name, description=description, location=location, user_id=current_user.id)
        db.session.add(project)
        db.session.flush()  # ID'yi almak için

        # Proje malzemelerini ekle
        for comp_id, amount in selected_components:
        ...
        db.session.commit()
        flash('Proje başarıyla oluşturuldu ve malzemeler ödünç alındı.', 'success')
        return redirect(url_for('my_projects'))

    return render_template('create_project.html', components=components_by_type)

@app.route('/projelerim/<int:project_id>/duzenle', methods=['GET', 'POST'])
@login_required
def edit_project(project_id):
    ...
    if request.method == 'POST':
        ...
        # Onay durumu değiştirilmesin (user değiştiremez)
        
        total_items = int(request.form['total_items'])
        for i in range(1, total_items + 1):
            item_id = int(request.form.get(f'item_id_{i}'))
            new_amount = int(request.form.get(f'item_amount_{i}'))

            item = ProjectItem.query.get(item_id)
...
@app.route('/projelerim/<int:project_id>/sil', methods=['POST'])
@login_required
...
def admin_projects():
    if not current_user.is_admin:
        abort(403)
    projects = Project.query.filter_by(approved=False).all()
    return render_template('admin/admin_projects.html', projects=projects)


@app.route('/admin/proje_onayla/<int:project_id>', methods=['POST'])
@login_required
def approve_project(project_id):
    ...
    db.session.commit()
    flash("Proje onaylandı.", "success")
    return redirect(url_for('admin_projects'))

@app.route('/admin/istekler', methods=['GET', 'POST'])
@login_required
def admin_requests():
    if not current_user.is_admin:
        abort(403)
