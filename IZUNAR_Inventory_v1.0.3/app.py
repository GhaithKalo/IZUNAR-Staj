...
def index():
    ...
    all_types = []
    all_categories = db.session.query(Component.category).distinct().all()

    if search_query:
        components = Component.query.options(joinedload(Component.inventory_items)).filter(
            or_(
                Component.name.ilike(f'%{search_query}%'),
                Component.type.ilike(f'%{search_query}%'),
                Component.code.ilike(f'%{search_query}%')
            )
        ).all()
        ...
        components = Component.query.options(joinedload(Component.inventory_items)).filter(
            func.lower(Component.category) == selected_category.lower()
        ).all()
...
def edit_user(user_id):
    ...
    if request.method == 'POST':
        ...
        if new_password:
            ...
            user.set_password(new_password)  # Burada set_password() kullanıldı
...
def add_component():
    ...
    if request.method == 'POST':
        ...
        # Demirbaş için seri numaraları işleme
        serial_numbers = []
        if category and category.lower() == 'demirbas':
            if not owner_prefix:
                flash("Demirbaşlar için sahiplik kısaltması (prefix) zorunludur.")
                return render_template('add.html', types=types, existing_tags=existing_tags, selected_tags=selected_tags)

            serial_numbers = [f"{owner_prefix}-{sn.strip()}" for sn in serial_numbers_raw.split(',') if sn.strip()]
            if len(serial_numbers) != quantity_int:
                flash("Seri numarası sayısı ile miktar eşleşmeli.")
                return render_template('add.html', types=types, existing_tags=existing_tags, selected_tags=selected_tags)
        ...
        db.session.flush()  # ID gerekiyor

        # Demirbaş ise InventoryItem oluştur
        if category and category.lower() == 'demirbas':
            for sn in serial_numbers:
                if InventoryItem.query.filter_by(serial_number=sn).first():
                    flash(f"Bu seri numarası zaten var: {sn}")
                    return render_template('add.html', types=types, existing_tags=existing_tags, selected_tags=selected_tags)
...
def component_detail(comp_id):
    ...
    inventory_items = InventoryItem.query.filter_by(component_id=comp_id).all()  # Seri numaralarını al
    ...
    return render_template('component_detail.html',
                           ...
                           inventory_items=inventory_items)  # Listeyi template'e gönder
...
def update_stock(comp_id):
    ...
    if component.category.lower() == "demirbas":
...
def borrow(comp_id):
    ...
    location = request.form.get('location', '').strip()
    serial_numbers = request.form.getlist('serial_number')  # Çoklu seri numarası desteği
    ...
    if component.category.lower() == "demirbas":
        if not serial_numbers or len(serial_numbers) != amount:
            flash("Seçilen seri numarası sayısı, miktarla aynı olmalıdır.")
            return redirect(url_for('index'))

        for sn in serial_numbers:
            inventory_item = InventoryItem.query.filter_by(serial_number=sn, component_id=comp_id).first()
            if not inventory_item:
                flash(f"Seri numarası '{sn}' geçersiz.")
                return redirect(url_for('index'))
            if inventory_item.assigned_to:
                flash(f"Seri numarası '{sn}' zaten atanmış.")
                return redirect(url_for('index'))

            inventory_item.assigned_to = current_user.username
    ...
    log = BorrowLog(
        ...
        amount=amount,
        location=location,
        serial_number=", ".join(serial_numbers) if component.category.lower() == "demirbas" else None
    )
...
def my_borrowed():
    ...
    borrowed_info = []
    for comp, borrowed in results:
        inventory_items = InventoryItem.query.filter_by(component_id=comp.id, assigned_to=current_user.username).all()
        borrowed_info.append((comp, borrowed, inventory_items))

    return render_template('my_borrowed.html', borrowed_items=borrowed_info)
...
if __name__ == '__main__':
    with app.app_context():
        ...
        if not admin:
            user = User(username="admin",password="envanter.izu", role="admin")
            user.set_password("envanter.izu")
            db.session.add(user)
            db.session.commit()
